...
export class SignupComponent implements OnInit {
  ...
  onSubmitClient()
  {
    ...
    if(this.nameValide && this.passwordValid && this.emailValid && this.passwordRepValide)
    {
      ...
      {
        var newClient = new Client(this.signupClientForm.value.name,this.signupClientForm.value.email,this.signupClientForm.value.motDePasse,this.signupClientForm.value.phone);
        //console.log(this.signupClientForm.value.name);
        this.clientService.addNewUser(newClient).subscribe(
          (data : any) =>
          {
            this.messages = data;
            if(/*this.messages[0].add ||*/true) // Create problems 
            {
              Swal.fire({
                position: 'center',
